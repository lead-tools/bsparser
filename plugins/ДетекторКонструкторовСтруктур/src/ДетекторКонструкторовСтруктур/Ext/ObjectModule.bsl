			...
			Если Идентификатор.Аргументы = Неопределено
				...
				Если ПустаяСтрока(Отступ) Тогда
					...
					Для Индекс = 0 По Ключи.Количество() - 1 Цикл
						...
						ПервыйТокен = ТаблицаТокенов[Идентификатор.Начало];
						ПоследнийТокен = ТаблицаТокенов[Идентификатор.Конец];
						Буфер.Добавить(Сред(
							Исходник,
							ПервыйТокен.Позиция,
					...
					Замена(Текст, ПервыйТокен.Позиция, ПоследнийТокен.Позиция + ПоследнийТокен.Длина - ПервыйТокен.Позиция);

...
Процедура ПокинутьОператорПрисваивания(ОператорПрисваивания, Стек, Счетчики) Экспорт
	...
	Места[Объявление] = ОператорПрисваивания.Место;	
...
Процедура ПосетитьВыражениеИдентификатор(ВыражениеИдентификатор, Стек, Счетчики) Экспорт
	Перем Объявление, Операция;
	Если ВыражениеИдентификатор.Хвост.Count() = 0
...
Процедура ПокинутьОбъявлениеМетода(ОбъявлениеМетода, Стек, Счетчики) Экспорт
	...
	Для Каждого Элемент Из Переменные Цикл 
		Если Не СтрНачинаетсяС(Элемент.Значение, "Чтение") Тогда 
			...
			Ошибка.Текст = Текст;
			Ошибка.НомерСтроки = Места[Элемент.Ключ].НомерПервойСтроки;
			Ошибка.Позиция = Места[Элемент.Ключ].Позиция;		
		КонецЕсли;	
	КонецЦикла;

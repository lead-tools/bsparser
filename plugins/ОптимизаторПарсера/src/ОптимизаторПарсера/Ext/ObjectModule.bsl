...
Функция Закрыть() Экспорт
	...
	Для Каждого Вызов Из ТочкиВызовов Цикл
		...
		Для Каждого Аргумент Из АргументыСтруктур[Вызов.Голова.Имя] Цикл
			...
			Если Аргумент.Тип = Узлы.ВыражениеИдентификатор
				...
				Если ИндексПараметра < Вызов.Аргументы.Количество() Тогда
					...
					Если Подмена = Неопределено Тогда
						...
						ПервыйТокен = ТаблицаТокенов[Выражение.Начало];
						ПоследнийТокен = ТаблицаТокенов[Выражение.Конец];
						
						Аргументы.Добавить(Сред(Исходник, ПервыйТокен.Позиция, ПоследнийТокен.Позиция + ПоследнийТокен.Длина - ПервыйТокен.Позиция));
			...
			Иначе
				...
				ПервыйТокен = ТаблицаТокенов[Аргумент.Начало];
				ПоследнийТокен = ТаблицаТокенов[Аргумент.Конец];
	...
	Для Каждого Вызов Из НовыеВызововы Цикл
		...
		ПервыйТокен = ТаблицаТокенов[Вызов.Ключ.Начало];
		ПоследнийТокен = ТаблицаТокенов[Вызов.Ключ.Конец];
		
		НоваяСтрока = ТаблицаЗамен.Добавить();
		НоваяСтрока.Текст = Вызов.Значение;
		НоваяСтрока.Начало = ПервыйТокен.Позиция;
		НоваяСтрока.Конец = ПоследнийТокен.Позиция + ПоследнийТокен.Длина;

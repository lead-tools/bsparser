...
**Внимание! Проект в процессе перевода исходного кода на русский язык.**

Данный проект представляет из себя набор внешних обработок для платформы 1С:Предприятие версии 8.3.15
...
Общее представление можно получить в этой статье: [Зачем нужно AST](https://ps-group.github.io/compilers/ast)
...
В данном плагине ошибки просто собираются в массив. Список ошибок можно получить вызовом Закрыть() у обработки плагина после того как визитер закончит обход.
...
Это генератор байткода, который работает (почти) идентично платформенному.
...
* /backends - исходники обработок-бакендов
...
Для использования обработок необходима либо установленная платформа 1С:Предприятие версии 8.3.15, либо интерпретатор [OneScript](https://github.com/EvilBeaver/OneScript)
...
Исходники обработок в данном проекте выгружены стандартными средствами конфигуратора платформы 1С:Предприятие версии 8.3.15. Для сборки вы можете просто открыть файл `xml` в конфигураторе как есть и пересохранить в формате `epf`
...
2. Открыть обработку build/gui.epf в управляемом приложении любой файловой базы (обработка ПарсерВстроенногоЯзыка.epf должна лежать рядом с gui.epf)
...
7. Для запуска плагина нужно в поле `Вывод` выбрать `Плагины` и указать файл обработки-плагина. Затем нажать `Выполнить`. В поле `Результат` будет выведен результат работы плагина.

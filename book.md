...
Пример формирования окружения смотрите в скрипте: [/oscript/test5.os](/oscript/test5.os)
...
* *`Окружение(ВнешнееОкружение): Структура`* - конструктор; возвращает новое окружение.

  Параметры:
  * `ВнешнееОкружение: Структура` -- внешнее окружение по отношению к создаваемому.

* *`ЭлементОкружения(Имя, Объявление): Структура`* - конструктор; возвращает новый элемент окружения.

  Параметры:
  * `Имя: Строка` -- имя элемента (переменной или метода).
  * `Объявление: Структура` -- объявление элемента (переменной или метода).

* `ОбъявлениеГлобальногоОбъекта(Имя, Доступность): Структура` - конструктор; возвращает новый глобальный объект.
...
* `ОбъявлениеГлобальногоМетода(Имя, ВозвращаетЗначение, Параметры, Доступность): Структура` - конструктор; возвращает новый глобальный метод.
...
* `Доступность(Клиент, ВнешнееСоединение, МобильноеПриложение, МобильныйКлиент, МобильныйСервер, Сервер, ТолстыйКлиент, ТонкийКлиент, ВебКлиент, Интеграция): Структура` - конструктор; возвращает новую структуру доступности.
...
* `УстановитьИсходник(Исходник)` - устанавливает исходник напрямую без выполнения анализа.

  Параметры:
  * `Исходник: Строка` -- исходный код, который нужно установить в парсер.

* `УстановитьТаблицуТокенов(ТаблицаТокенов)` - устанавливает таблицу токенов напрямую без выполнения анализа.

  Параметры:
  * `ТаблицаТокенов: ТаблицаЗначений` -- таблица токенов, которую нужно установить в парсер.

* `УстановитьТаблицуОшибок(ТаблицаОшибок)` - устанавливает таблицу ошибок напрямую без выполнения анализа.
...
2. В модуль объекта обработки скопировать шаблон с сайта: https://lead-tools.github.io/bsparser

...
* Пуск(Исходник: Строка, Плагины: Массив или Обработка, Параметры: Соответствие, Окружение: Структура): Массив - выполняет разбор исходника в указанном окружении, обход AST с указанными плагинами и указанными параметрами плагинов, возвращает результаты (обычно текстовые) работы плагинов.
...
* Подключить(Плагины: Массив или Обработка) - подключает плагины к парсеру.
...
* Типы(): Структура - возвращает перечисление допустимых типов узлов AST (например: `Типы.ОператорПрисваивания`).

...
Процедура ЗаполнитьУзел(СтрокаДерева, Узел)
	...
	Если ТипЗнч(Узел) = Тип("Структура") И Узел.Свойство("Начало", Начало) И Узел.Свойство("Конец", Конец) Тогда
		...
		ПервыйТокен = ЭтотОбъект.Токены[Начало];
		ПоследнийТокен = ЭтотОбъект.Токены[Конец];
		СтрокаДерева.Позиция = ПервыйТокен.Позиция;
		СтрокаДерева.Длина = ПоследнийТокен.Позиция + ПоследнийТокен.Длина - ПервыйТокен.Позиция;
	КонецЕсли;

...
Функция Разобрать(Парсер, ТекстМодуля)
	...
	Если ЭтотОбъект.ИспользоватьКэшАСД И НовыйХэшИсходника = ЭтотОбъект.ХэшИсходника Тогда
		...
		ТаблицаТокенов.Очистить();
		Для Каждого Строка Из ПолучитьИзВременногоХранилища(ЭтотОбъект.АдресКэшаТокенов) Цикл
			ЗаполнитьЗначенияСвойств(ТаблицаТокенов.Добавить(), Строка);
		КонецЦикла;
	...
	Иначе
		...
		Попытка 
			...
			Парсер.СоздаватьГлобальноеОкружение = ЭтотОбъект.СоздаватьГлобальноеОкружение;
			...
			ЭтотОбъект.АдресКэшаОшибок = ПоместитьВоВременноеХранилище(Парсер.Ошибки(), ЭтотОбъект.УникальныйИдентификатор);
...
Процедура УстановитьВидимостьЭлементов(ЭтотОбъект)
	...
	Элементы.СтраницаРезультатТокены.Видимость = (ЭтотОбъект.Вывод = "Дерево" Или ЭтотОбъект.Вывод = "Токены");	

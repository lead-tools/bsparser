...
// Генератор исходного кода по АСД
...
Процедура ПосетитьОбъявление(Объявление)
    Перем Тип;
	УстановитьОтступ(Объявление.Начало);
	Тип = Объявление.Тип;
...
Процедура ПосетитьОбъявлениеМетода(ОбъявлениеМетода)
	Если ОбъявлениеМетода.Сигнатура.Тип = Узлы.СигнатураФункции Тогда
		...
		Если ОбъявлениеМетода.Переменные.Количество() > 0 Тогда
			...
			//ИндексПоследнегоТокена = ОбъявлениеМетода.Сигнатура.Конец;
		КонецЕсли;
		ПосетитьОператоры(ОбъявлениеМетода.Операторы);
		УстановитьОтступ(ОбъявлениеМетода.Конец);
...
Процедура ПосетитьВыражение(Выражение)
    Перем Тип;
	УстановитьОтступ(Выражение.Начало);
	Тип = Выражение.Тип;
...
Процедура ПосетитьВыражениеИдентификатор(ВыражениеИдентификатор)
	...
	Если ВыражениеИдентификатор.Аргументы <> Неопределено Тогда
		...
		Отступ = Отступ - 1; // <<
		УстановитьОтступ(ВыражениеИдентификатор.Конец);
		Результат.Добавить(")");
	КонецЕсли;
...
Процедура ПосетитьВыражениеБинарное(ВыражениеБинарное)
	...
	УстановитьОтступ(ВыражениеБинарное.ПравыйОперанд.Начало);
...
Процедура ПосетитьВыражениеНовый(ВыражениеНовый)
	...
	Если ВыражениеНовый.Аргументы.Количество() > 0 Тогда
		...
		Отступ = Отступ - 1; // <<
		УстановитьОтступ(ВыражениеНовый.Конец);
		Результат.Добавить(")");
	КонецЕсли;
...
Процедура ПосетитьВыражениеСкобочное(ВыражениеСкобочное)
	...
	Отступ = Отступ - 1; // <<
	УстановитьОтступ(ВыражениеСкобочное.Конец);
	Результат.Добавить(")");
...
Процедура ПосетитьВыражениеСтроковое(ВыражениеСтроковое)
	Если ВыражениеСтроковое.Элементы.Количество() > 1 Тогда
		Для Каждого Литерал Из ВыражениеСтроковое.Элементы Цикл
			УстановитьОтступ(Литерал.Начало);
...
Процедура ПосетитьОператор(Оператор)
	УстановитьОтступ(Оператор.Начало);
	Тип = Оператор.Тип;
...
Процедура ПосетитьОператорПока(ОператорПока)
	...
	УстановитьОтступ(ОператорПока.Конец);
...
Процедура ПосетитьОператорДля(ОператорДля)
	...
	УстановитьОтступ(ОператорДля.Конец);
	Результат.Добавить("КонецЦикла;");
...
Процедура ПосетитьОператорДляКаждого(ОператорДляКаждого)
	...
	УстановитьОтступ(ОператорДляКаждого.Конец);
	Результат.Добавить("КонецЦикла;");
...
Процедура ПосетитьОператорПопытка(ОператорПопытка)
	...
	УстановитьОтступ(ОператорПопытка.Исключение.Начало);
	ПосетитьОператорИсключение(ОператорПопытка.Исключение);
	УстановитьОтступ(ОператорПопытка.Конец);
...
Процедура ПосетитьВыражениеПрепроцессора(ВыражениеПрепроцессора)
	Перем Тип;
	УстановитьОтступ(ВыражениеПрепроцессора.Начало);
...
Процедура ПосетитьВыражениеПрепроцессораСкобочное(ВыражениеПрепроцессораСкобочное)
	...
	Отступ = Отступ - 1; // <<
	УстановитьОтступ(ВыражениеПрепроцессораСкобочное.Конец);
	Результат.Добавить(")");
...
Процедура УстановитьОтступ(ИндексТокена)
	НомерСтроки = ТаблицаТокенов[ИндексПоследнегоТокена].НомерСтроки;
	Пока ИндексПоследнегоТокена < ИндексТокена Цикл
		ИндексПоследнегоТокена = ИндексПоследнегоТокена + 1;
		Следующий = ТаблицаТокенов[ИндексПоследнегоТокена];
		Если НомерСтроки <> Следующий.НомерСтроки Тогда
			НомерСтроки = Следующий.НомерСтроки;
			Результат.Добавить(Символы.ПС); Отступ();
		КонецЕсли;
		Если Следующий.Токен = Токены.Комментарий Тогда
			Результат.Добавить(" //" + Сред(Исходник, Следующий.Начало, Следующий.Конец - Следующий.Начало));
		КонецЕсли;
	КонецЦикла;
...
Процедура ПосетитьПеременные(Переменные)
	...
	Для Каждого ОбъявлениеПеременной Из Переменные Цикл
		Если ОбъявлениеПеременной.Свойство("Экспорт") И ОбъявлениеПеременной.Экспорт Тогда
			Буфер.Добавить(ОбъявлениеПеременной.Имя + " Экспорт");
		Иначе
			Буфер.Добавить(ОбъявлениеПеременной.Имя);
		КонецЕсли;
		ИндексПоследнегоТокена = ОбъявлениеПеременной.Конец;
	КонецЦикла;
	...
	Для Каждого Элемент Из Хвост Цикл
		Если Элемент.Тип = Узлы.ВыражениеПоле Тогда
			...
			Если Элемент.Аргументы <> Неопределено Тогда
				...
				Отступ = Отступ - 1; // <<
				УстановитьОтступ(Элемент.Конец);
				Результат.Добавить(")");
			КонецЕсли;

...
Процедура ПосетитьОбъявление(Объявление)
    Перем Тип;
	УстановитьОтступ(ТаблицаТокенов[Объявление.Начало].НомерСтроки);
	Тип = Объявление.Тип;
...
Процедура ПосетитьОбъявлениеМетода(ОбъявлениеМетода)
	Если ОбъявлениеМетода.Сигнатура.Тип = Узлы.СигнатураФункции Тогда
		...
		Если ОбъявлениеМетода.Переменные.Количество() > 0 Тогда
			...
			ПоследняяСтрока = ТаблицаТокенов[ОбъявлениеМетода.Сигнатура.Конец].НомерСтроки + 1;
		КонецЕсли;
		ПосетитьОператоры(ОбъявлениеМетода.Операторы);
		УстановитьОтступ(ТаблицаТокенов[ОбъявлениеМетода.Конец].НомерСтроки);
...
Процедура ПосетитьВыражение(Выражение)
    Перем Тип;
	УстановитьОтступ(ТаблицаТокенов[Выражение.Начало].НомерСтроки);
	Тип = Выражение.Тип;
...
Процедура ПосетитьВыражениеИдентификатор(ВыражениеИдентификатор)
	...
	Если ВыражениеИдентификатор.Аргументы <> Неопределено Тогда
		...
		Отступ = Отступ - 1; // <<
		УстановитьОтступ(ТаблицаТокенов[ВыражениеИдентификатор.Конец].НомерСтроки);
		Результат.Добавить(")");
	КонецЕсли;
...
Процедура ПосетитьВыражениеБинарное(ВыражениеБинарное)
	...
	УстановитьОтступ(ТаблицаТокенов[ВыражениеБинарное.ПравыйОперанд.Начало].НомерСтроки);
...
Процедура ПосетитьВыражениеНовый(ВыражениеНовый)
	...
	Если ВыражениеНовый.Аргументы.Количество() > 0 Тогда
		...
		Отступ = Отступ - 1; // <<
		УстановитьОтступ(ТаблицаТокенов[ВыражениеНовый.Конец].НомерСтроки);
		Результат.Добавить(")");
	КонецЕсли;
...
Процедура ПосетитьВыражениеСкобочное(ВыражениеСкобочное)
	...
	Отступ = Отступ - 1; // <<
	УстановитьОтступ(ТаблицаТокенов[ВыражениеСкобочное.Конец].НомерСтроки);
	Результат.Добавить(")");
...
Процедура ПосетитьВыражениеСтроковое(ВыражениеСтроковое)
	Если ВыражениеСтроковое.Элементы.Количество() > 1 Тогда
		Для Каждого Литерал Из ВыражениеСтроковое.Элементы Цикл
			УстановитьОтступ(ТаблицаТокенов[Литерал.Начало].НомерСтроки);
...
Процедура ПосетитьОператор(Оператор)
	УстановитьОтступ(ТаблицаТокенов[Оператор.Начало].НомерСтроки);
	Тип = Оператор.Тип;
...
Процедура ПосетитьОператорЕсли(ОператорЕсли)
    ...
    Если ОператорЕсли.ИначеЕсли <> Неопределено Тогда
        Для Каждого ОператорИначеЕсли Из ОператорЕсли.ИначеЕсли Цикл
			УстановитьОтступ(ТаблицаТокенов[ОператорИначеЕсли.Начало].НомерСтроки);
			ПосетитьОператорИначеЕсли(ОператорИначеЕсли);
        КонецЦикла;
    КонецЕсли;
    Если ОператорЕсли.Иначе <> Неопределено Тогда
		УстановитьОтступ(ТаблицаТокенов[ОператорЕсли.Иначе.Начало].НомерСтроки);
		ПосетитьОператорИначе(ОператорЕсли.Иначе);
	КонецЕсли;
	УстановитьОтступ(ТаблицаТокенов[ОператорЕсли.Конец].НомерСтроки);
...
Процедура ПосетитьОператорПока(ОператорПока)
	...
	УстановитьОтступ(ТаблицаТокенов[ОператорПока.Конец].НомерСтроки);
...
Процедура ПосетитьОператорДля(ОператорДля)
	...
	УстановитьОтступ(ТаблицаТокенов[ОператорДля.Конец].НомерСтроки);
	Результат.Добавить("КонецЦикла;");
...
Процедура ПосетитьОператорДляКаждого(ОператорДляКаждого)
	...
	УстановитьОтступ(ТаблицаТокенов[ОператорДляКаждого.Конец].НомерСтроки);
	Результат.Добавить("КонецЦикла;");
...
Процедура ПосетитьОператорПопытка(ОператорПопытка)
	...
	УстановитьОтступ(ТаблицаТокенов[ОператорПопытка.Исключение.Начало].НомерСтроки);
	ПосетитьОператорИсключение(ОператорПопытка.Исключение);
	УстановитьОтступ(ТаблицаТокенов[ОператорПопытка.Конец].НомерСтроки);
...
Процедура ПосетитьВыражениеПрепроцессора(ВыражениеПрепроцессора)
	Перем Тип;
	УстановитьОтступ(ТаблицаТокенов[ВыражениеПрепроцессора.Начало].НомерСтроки);
...
Процедура ПосетитьВыражениеПрепроцессораСкобочное(ВыражениеПрепроцессораСкобочное)
	...
	Отступ = Отступ - 1; // <<
	УстановитьОтступ(ТаблицаТокенов[ВыражениеПрепроцессораСкобочное.Конец].НомерСтроки);
	Результат.Добавить(")");
	...
	Для Каждого Элемент Из Хвост Цикл
		Если Элемент.Тип = Узлы.ВыражениеПоле Тогда
			...
			Если Элемент.Аргументы <> Неопределено Тогда
				...
				Отступ = Отступ - 1; // <<
				УстановитьОтступ(ТаблицаТокенов[Элемент.Конец].НомерСтроки);
				Результат.Добавить(")");
			КонецЕсли;
